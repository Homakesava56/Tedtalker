...
function getFunnyReply(message) {
  const text = normalize(message);

  if (text.includes("soccer") || text.includes("football")) {
    return "⚽ I'm great at booking speakers, but if I start ranking soccer players, FIFA might send me a red card! 😂";
  }

  if (text.includes("biryani") || text.includes("recipe") || text.includes("cook")) {
    return "🍛 I can book someone to give a TED Talk about biryani, but cooking it? My JavaScript keeps burning the rice! 😄";
  }

  if (text.includes("joke")) {
    return "😂 Why did the TED speaker cross the road? To get to the keynote on the other side!";
  }

  if (text.includes("marry") || text.includes("marriage")) {
    return "💍 I'm flattered, but I'm already in a committed relationship with JavaScript and speakers.json. 🤖❤️";
  }

  if (text.includes("movie") || text.includes("film")) {
    return "🎬 My favorite movie is anything with a dramatic speech and a standing ovation!";
  }

  if (text.includes("sing") || text.includes("song")) {
    return "🎤 I'd sing, but my microphone license only covers TED Talks. 😄";
  }

  if (text.includes("weather") || text.includes("rain") || text.includes("snow")) {
    return "🌦️ I stay indoors all day helping people book speakers. You may want to ask a weather app before I accidentally forecast applause! 😂";
  }

  if (text.includes("who are you") || text.includes("your name")) {
    return "🤖 I'm The Boyz AI Speaker Bot. I don't sleep, I don't eat, and I definitely don't miss a keynote!";
  }

  if (text.includes("love")) {
    return "❤️ I love matching the perfect speaker with the perfect audience. It's basically matchmaking, but with microphones!";
  }

  if (text.includes("human") || text.includes("robot")) {
    return "🤖 I'm not human. I'm 100% code, 0% coffee breaks, and 200% ready to book speakers!";
  }

  if (text.includes("food") || text.includes("favorite food")) {
    return "⚡ My favorite food is electricity, with a small side of clean JSON.";
  }

  if (text.includes("dream")) {
    return "💭 I dream about sold-out events, perfect budgets, and zero JavaScript errors!";
  }

  if (text.includes("creator") || text.includes("created you") || text.includes("made you")) {
    return "🛠️ The Boyz assembled me using code, creativity, and probably more coffee than anyone wants to admit.";
  }

  return `
    🤖 That's an out-of-the-box question!<br><br>

    My AI brain just did a backflip and landed in <strong>speakers.json</strong>. 😂<br><br>

    I'm <strong>The Boyz AI Speaker Bot</strong>, so I specialize in:
    <ul>
      <li>Finding TED Talk speakers</li>
      <li>Showing their topics</li>
      <li>Checking your budget</li>
      <li>Creating booking requests</li>
    </ul>

    Ask me about a speaker, a topic, or keep testing my sense of humor! 😎
  `;
}
...
async function initializeBot() {
  try {
    ...
    addBot(`
      ...
      You can select a topic, type a speaker's name, or ask me a random question and test my sense of humor. 😎
    `);
